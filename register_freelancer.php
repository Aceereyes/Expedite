<?php
    ...
    if(isset($_POST['btnRegister'])) {
        ...
        $validation = $validator->validate($_POST, [
            ...
            'email:Email Address'                               => 'required|email|unique:freelancers,email',
        ...
        if($validation->fails()) {
            ...
            $freelancer = \App\Models\Freelancer::create([
...
?>
 <script>
        ...
        var title = 'Requirements: At least 8 characters, One lowercase, One Uppercase, One number';

        // Define the expected requirements
        ...
        // Extract the requirements from the title
        var pattern = /(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}/g;
        var requirements = title.match(pattern) || [];

        // Find the missing requirements
        var missingRequirements = expectedRequirements.filter(function(requirement) {
            return requirements.indexOf(requirement) === -1;
        });

        // Display the missing requirements
        var requirementsDiv = document.getElementById('requirements');
        missingRequirements.forEach(function(requirement) {
            var p = document.createElement('p');
            p.textContent = 'Missing requirement: ' + requirement;
            requirementsDiv.appendChild(p);
        });
    </script>

    <div class="page page-center" style="background-image: url(assets/images/bglogin4.png); background-size: cover; background-repeat: no-repeat; background-position: center center;">
        <div class="container container-tight py-4">
            <div class="card card-md" style="border-radius: 70px;">
                <div class="card-body">
                    <h2 class="h2 text-center mb-4">Become One Of Our Freelancers</h2>
                    <form method="POST" autocomplete="off">
                        <div class="mb-3">
                            <label class="form-label">First Name</label>
                            <input type="text" name="firstName" class="form-control" placeholder="Enter Your First Name" autocomplete="off" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Last Name</label>
                            <input type="text" name="lastName" class="form-control" placeholder="Enter Your Last Name" autocomplete="off" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="password" name="password_confirmation" class="form-control" placeholder="Confirm Your Password" autocomplete="off" title="You must Match the Password Given" id="confirm_password" required>
                ...
                <div class="hr-text">or</div>
                    <div class="card-body">
                        <a href="register_partner.php" class="btn btn-outline-secondary w-100 rounded-pill btn-cute" style="font-size: 16px; padding: 12px;">Register as Partner</a>
                    </div>
                </div>
            </div>
        </div>
