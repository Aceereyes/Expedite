<?php
    ...
    if(!isset($_GET['id'])) {
        ...
        redirect('freelancer_profile_other.php');
        exit();
    }

    $record = \App\Models\Freelancer\OtherAttachment::find($_GET['id']);

    if(isset($_POST['btnUpdate'])) {
        $record->update([
            'type' => $_POST['type'],
            'speaking' => $_POST['issuer'],
        ]);
                    ...
                    <form method="POST" class="col d-flex flex-column">
                        <div class="card-body pb-4">
                            <div class="row g-2 align-items-center mb-3">
                                <div class="col">
                                    <h2 class="page-title">Attachment</h2>
                                </div>
                                <div class="col-auto ms-auto d-print-none">
                                    <a href="freelancer_profile_other.php" class="btn">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" value="<?= $record->type ?>" name="type" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Issuer</label>
                                <input type="text" class="form-control" value="<?= $record->issuer ?>" name="issuer" required>
                            </div>
                            <div class="input-group mb-3">
                                <input type="text" value="<?= $record->name; ?>" class="form-control" disabled>
                                <a class="btn btn-outline-secondary" href="uploads/<?= $record->name; ?>" target="_blank">Preview</a>
                            </div>
                        </div>
