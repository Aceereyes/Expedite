...
<?php include '_partial/partial_navbar.php'; ?>
                ...
                <div class="x_panel">
                  <div class="x_title">
					...
					if(isset($_POST['vision_save'])){
						echo '<img src="_photos/loading.gif"><meta http-equiv="refresh" content="5; url=vision.php"> Vision';
						
					}else{
					echo '<i class="fa fa-desktop"></i> Vision';
					}
					?>
					</h2>
                    <div class="clearfix"></div>
	...
	if(isset($_POST['vision_save'])){
		$vision = $_POST['vision'];
		mysqli_query($conn, "UPDATE aboutus SET vision = '$vision' WHERE vision != ''");
		echo ''.$vision.'';
	}
	?>
	<form method = "POST">
	<?php
		...
		$vision_sel = mysqli_query($conn, "SELECT * FROM aboutus WHERE vision != ''");
		$vision_view = mysqli_fetch_array($vision_sel);
		$vision = $vision_view['vision'];
		?>
		 <input name="vision" id="inp_htmlcode" type="hidden" />
			<?php echo $vision; ?>
