...
if($user_staff_password == $repassword){
...
$user_staff_account_created_by = 'Guest Account';
...
mkdir('adminpage/_photos/profilephotos/admins/'.$user_staff_first_name.''.$user_staff_middle_name.''.$user_staff_last_name.'');

move_uploaded_file($_FILES["user_staff_photo"]["tmp_name"],"adminpage/_photos/profilephotos/admins/".$user_staff_first_name."".$user_staff_middle_name."".$user_staff_last_name."/" . $_FILES["user_staff_photo"]["name"]);
...
'ON')");
	echo '
		<strong>'.$user_staff_first_name.' '.$user_staff_last_name.'</strong> Has been registered to our system as <strong>'.$priviledge_level.'</strong>
					...
					User\'s Information has been successfully sent to <b>'.$user_staff_email.'</b>. </br>
					<script>
						var timeleft = 3;
...
// Output the image
imagepng($im, 'adminpage/_photos/profilephotos/admins/'.$user_staff_first_name.''.$user_staff_middle_name.''.$user_staff_last_name.'/'.$full_cut_strings.'.png');
...
mysqli_close($conn);
	...
	echo '<strong>Password dont match.</strong>';
}
?>