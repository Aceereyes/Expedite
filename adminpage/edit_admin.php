...
?>
						<div class="col-md-4 col-sm-4 col-xs-12 form-group has-feedback">
							<?php date_default_timezone_set('Singapore');$startDate = time();$year = date('Y', strtotime('-1 day', $startDate));?>
						...
						<div class="col-md-4 col-sm-4 col-xs-12 form-group has-feedback">
							<?php date_default_timezone_set('Singapore');$startDate = time();$today = date('Y-m-d', strtotime('-1 day', $startDate));?>
					...
					<h2>Personal Information</h2>
						...
						<?php date_default_timezone_set('Hongkong'); $startDate = time(); $today = date('Y-m-d', strtotime('-2 day', $startDate));?>
					...
					-->
						<div class="col-md-6 col-sm-6 col-xs-12 form-group has-feedback">
							<label>Mobile Authenticator:</label>
							<select  class="form-control" name="user_admin_mobile_authenticator" id = "user_admin_mobile_authenticator" >
								<option value = "ON_MOB_AUTH">Enabled (Turn ON Mobile Authenticator)</option>
								<option value = "OFF_MOB_AUTH" selected>Disabled (Turn OFF Mobile Authenticator) </option>
							</select>
						</div>
						<div class="col-md-6 col-sm-6 col-xs-12 form-group has-feedback">
						...
						<div class="col-md-12 col-sm-12 col-xs-12 input-group">
							<label>PRIVILEDGE: Giving rights to ADD/MODIFY/DELETE/VIEW every Administrator, Staff, Faculty and Student:</label>
								<div class="radio">
									<label>
										<input type="radio" class="flat" name="priviledge_of_users" value = "Full Control"> Full Control <i>(This user can ADD/MODIFY/DELETE/VIEW/ACCOUNT MANAGEMENT every users in this system)</i>
									</label>
								</div>
								<div class="radio">
									<label>
										<input type="radio" class="flat" name="priviledge_of_users" value = "Staff, Faculty and Student Only"> Admin Staff Only <i>(This user can ADD/MODIFY/DELETE/VIEW every users except Administrators)</i>
									</label>
								</div>
								<div class="radio">
									<label>
										<input type="radio" class="flat" name="priviledge_of_users" value = "Faculty and Student Only"> Faculty/Teachers Only <i>(This user can ADD/MODIFY/VIEW STUDENT GRADES & INFORMATIONS)</i>
									</label>
								</div>
								<div class="radio">
									<label>
										<input type="radio" class="flat" name="priviledge_of_users" value = "Student Only"> Student Only <i>(This user can VIEW OWN GRADES & PERSONAL INFORMATION only Students)</i>
									</label>
								</div>
								<div class="radio">
									<label>
										<input type="radio" class="flat" name="priviledge_of_users" value = "View Only"> Parent Only <i>(This user is only allowed to view informations)</i>
									</label>
								</div>
								<div class="radio">
									<label>
						...
						<div class="col-md-12 col-sm-12 col-xs-12 form-group has-feedback">
							<label for = "user_admin_signature">Signature:</label>
							<input  type="file" class="form-control" name = "user_admin_signature" id="user_admin_signature" accept="image/gif, image/jpeg, image/png">
						</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$.notify({
			...
			message: "<b>SYSTEM NOTIFICATION</b><br>Hi <b><?php echo @$user_admin_first_name; echo ' '; echo @$user_admin_last_name; ?></b>, You have succesfully created an <b><?php echo $priviledge_of_users; ?></b> account.<br>"
			},{
				type: "success",
				timer: 20000
            });
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$.notify({
			icon: "fa fa-check-circle",
			message: "<b>SYSTEM NOTIFICATION</b><br>All Admins will be notified about <b><?php echo $user_admin_first_name; echo ' '; echo $user_admin_last_name;?>'s <?php echo $priviledge_of_users; ?></b> account."
			},{
				type: "success",
				timer: 15000
            });
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$.notify({
			...
			message: "<b>SYSTEM NOTIFICATION</b><br>Welcome to <b>Paper Dashboard</b> - a beautiful Bootstrap freebie for<br> your next project."
			},{
				type: "info",
				timer: 20000
            });
