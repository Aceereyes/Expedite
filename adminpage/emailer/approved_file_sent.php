...
try {
	...
	$signatory_name_approved = '<b>'.$ADMIN_FIRSTNAME.' '.$ADMIN_MIDDLENAME.' '.$ADMIN_LASTNAME.'</b> of <b>'.$colleges_department_code.'</b> has been approved your form <b>'.$file_description.'</b>.<br/><br/> ';
	
	$mail->Body   .= '';
	$mail->Body   .= ''.$signatory_name_approved.'';
	if($note != ''){
		$mail->Body   .= '<i><b>NOTE:</b> '.$note.'</i>';
	}else{
		$mail->Body   .= '<i><b>NOTE:</b> NO NOTE.</i>';
	}
